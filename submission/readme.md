...
The harness reports the recursive size of every regular file retained under
`io/server_data/` as `Packed model weights`. Consequently, every Orion cache key
left in this directory contributes to the measurement; obsolete caches should
be removed before a benchmark run unless they are intentionally part of the
deployed server model.
